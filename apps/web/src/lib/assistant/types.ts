...
export type SummaryDashboardPayload = {
  ...
  expenseMinor: string;
  netMinor: string;
  /** Integer percent (0–100), null when income is zero. */
...
export type RecurringListPayload = {
  type: "recurring_list";
  periodLabel: string;
  assetCode: string;
  decimals: number;
...
export type AnomalyItemPayload = {
  description: string;
  amountMinor: string;
  dateLabel: string;
  /** Multiple of the merchant's median tx size, e.g. 3.4. */
  multipleOfMedian: number;
  medianMinor: string;
};

export type AnomalyListPayload = {
  type: "anomaly_list";
  periodLabel: string;
  assetCode: string;
  decimals: number;
  items: AnomalyItemPayload[];
  scope?: ScopeFlags;
};
...
type AssistantBase = {
  ...
  notice?: string;
  /**
   * Optional TL;DR generated by a second model call that sees only the
   * validated payload. Best-effort: failure to produce one is silent.
   */
  tldr?: string;
  /**
