...
Push ops: `VAPID_PRIVATE_KEY` accepts PKCS8 DER (`scripts/gen-vapid.mjs`) or a raw 32-byte scalar (`web-push` CLI format) — a format the sender can't parse 500s the test endpoint and silences all sends (2026-09-19 incident). Diagnose from the phone: Settings → Notifications → Test names the failing link (keys / reachability / subscriptions).
